...
describe('input persistence', () => {
  ...
  it('automatically migrates old ecotrace data to new keys', () => {
    globalRef.localStorage?.setItem('ecotrace:input', JSON.stringify(defaultFootprintInput));
    expect(loadInput()).toEqual(defaultFootprintInput);
    expect(globalRef.localStorage?.getItem('ecotrace:input')).toBeNull();
    expect(globalRef.localStorage?.getItem(STORAGE_KEYS.input)).not.toBeNull();
  });
});
